...
int Adventurer(struct gameState * state) {
	...
	while (drawnTreasure < 2) { //while less than 2 treasures have been drawn
            //THIS IS WHERE THINGS HAVE BEEN ALTERED FROM 	if (state->deckCount[currentPlayer] < 1)
		if (state->deckCount[currentPlayer] < 2) { //if the deck is empty we need to shuffle discard and add to deck
			shuffle(currentPlayer, state);
		}
    //printf("handCount in adv %d\n",state->handCount[currentPlayer]);
		drawCard(currentPlayer, state);
    //printf("handCount in adventurer %d\n",state->handCount[currentPlayer]);
		cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer] - 1]; //top card of hand is most recently drawn card.
		if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold){
			//printf("treasure %d",drawnTreasure);
			drawnTreasure++;}
		else {
...
int Council_room(struct gameState * state, int handPos) {
	...
	{
            //This line should be i != currentPlayer
		if ( i == currentPlayer )
		{
			drawCard(i, state);
		}
